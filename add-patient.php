<?php
	...
	{
		$error="";
		$msg="<span class='label label-success'>Patient Added Successfully</span>";
		// $msg="<br><span class='card' style= >Patient Added Successfully</span><br><br>";
                ...
                <form method="post" class="jNice">
                    ...
                    <?php
						if(isset($_POST['save']))
						{
							...
							$testresult=trim($_POST['testresult']);
							...
							elseif($testdate==""){ $error="<br><span class=error>Please enter the latest covid test date</span><br><br>"; }
							elseif($testresult==""){ $error="<br><span class=error>Please enter the Covid test results</span><br><br>"; }
							else
							{
								mysqli_query($server,"INSERT INTO patients (name,age,sex,blood_group,phone, testdate, testresult) VALUES ('$name','$age','$sex','$bg','$phone','$testdate','$testresult')");
								...
								mysqli_query($server,"INSERT INTO staff_to_pat (staff_id,pat_id) VALUES ('none','$row[pat_id]')");
					...
					?>
                    	<fieldset>
                        	<p><label>Patient Name:</label><input type="text" name="name" class="form-control" autofocus value="  " /></p>
                            ...
                            <p><label>Blood Group:</label><input type="text" name="bg" class="form-control" value=" " /></p>
                            <p><label>Phone Number:</label><input type="text" name="phone" class="form-control" value="  " /></p>
							<label>Latest Covid test Date</label>
							<input type="date" name="testdate" class="form-control"><br>
							<label>Covid-19 Test Result</label><br>
							<input type="radio" id="positive" name="testresult" value="postive">
							<label for="positive">Positive</label><br>
							<input type="radio" id="negative" name="testresult" value="negative">
							<label for="negative">Negative</label><br>
							<br>
                            <input type="submit" value="Save" name="save" class="btn btn-dark" />
                        </fieldset>
                    </form>
                        <br /><br />
						</div>
